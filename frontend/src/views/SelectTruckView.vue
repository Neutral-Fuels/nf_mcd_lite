      ...
      <div class="px-4 py-4">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <h1 class="text-2xl font-bold text-gray-900">Select Truck</h1>
          </div>
          <button
            @click="router.back()"
    ...
    <div v-else-if="error" class="px-4 py-8">
      <div class="bg-red-50 border border-red-200 rounded-lg p-4">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h3 class="text-sm font-medium text-red-800">Error loading trucks</h3>
            <p class="text-sm text-red-700 mt-1">{{ error }}</p>
            <button
              @click="fetchCustomerTrucks()"
    ...
    <div v-else class="px-4 py-4">
      ...
      <div v-if="filteredTrucks.length === 0" class="text-center py-12">
        ...
        <p class="mt-1 text-sm text-gray-500">
          {{ searchQuery ? 'Try adjusting your search' : 'No trucks available for your account' }}
        </p>
      </div>
      ...
      <div v-else class="space-y-3">
        <p class="text-sm text-gray-500 mb-3">
          {{ filteredTrucks.length }} truck{{ filteredTrucks.length !== 1 ? 's' : '' }} available
        </p>
        <TruckCard
